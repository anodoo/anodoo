...
class MergeOpportunity(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if record_ids:
            ...
            owner_type_set = set()
            user_id_set = set()
            owner_team_id_set = set()
            for opp in opps:
                owner_type_set.add(opp.owner_type)
                if opp.user_id:
                    user_id_set.add(opp.user_id.id)
                if opp.owner_team_id:
                    owner_team_id_set.add(opp.owner_team_id.id)
            ...
            if len(owner_team_id_set) == 1:
                result['owner_team_id'] = owner_team_id_set.pop()
