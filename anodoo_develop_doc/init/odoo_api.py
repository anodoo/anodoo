...
#api.py :The Odoo API module defines Odoo Environments and method decorators.
#很多注释的定义，Meta（type）的定义
# 1. 装饰器
# https://blog.csdn.net/tsoTeo/article/details/103807677
#  
# n 理解
# u 在装载一个类时，如果这个类的方法（或者类）有装饰器，则会调用装饰器函数。而根据装饰器函数的定义，它是返回了这个方法的包装方法。
# u 装饰器更多的是作为标注，关键在于什么调用装饰过的方法
# u 猜想：那什么时候调用这些类似depends的包装方法呢。应该是ORM体系内，如果判断depends的字段发生了变化，则从新计算，否则使用缓存。
# u 后续通过debug使用了这些装饰器的方法
# n Constrains
# u  
# n Depends
# u 对compute方法，标识该计算依赖的字段。
# n Depends_context
# u 对compute方法，标识该计算依赖的字段，是env.context中的一个属性
# n Returns
# n Onchange
# n Ormcache
# n Model_create_multi
# u 装饰create方法，对参数是一个，还是多个进行判断
#  
# https://pypi.org/project/decorator/
# python核心中没有装饰器，都是依靠第三方实现？？
#  
#  
# @api.*
# https://www.jianshu.com/p/137116e818e8
# https://blog.csdn.net/weixin_44611400/article/details/86645633
#  @http.route
#  
# 2. Environment
# n 创建，主要是cr游标，当前uid等
# n Context  dict
# u product.env.context.get('pricelist')
# l environment类提供了对ORM对象的封装，同时提供了对注册类的访问，记录集的缓存，以及管理重计算的数据结构。对于继承Model类来说可以直接通过self.env对environment进行操作。
# l 属性列表：
# l 1.user：返回当前用户
# l     self.env.user
# l 2.lang：返回当前语言代码
# l     self.env.lang
# l 3.in_draft:返回是否处于草稿状态
# l     self.env.in_draft
# l 4.in_onchange:返回是否处于on_change草稿模式
# l     self.env.in_onchange
# l 另外还有cr,registery,cache,prefetch,computed,dirty,todo,mode,all
# l 1.利用env[model]获取类对象，调用对象方法
# l     self.env['ir.model'].search([('state','!=','manual')])
# l 2.利用cr执行sql语句
# l     self.env.cr.execute(query,(value,))
# l https://www.sunpop.cn/odoo_environment_env/
# l  
# l  
# l https://www.cnblogs.com/ygj0930/p/10826382.html
# l https://blog.csdn.net/sinat_23931991/article/details/85
#  
# l Environments
#  
# n 在进程中，保持所有的env
...
#在create()函数上适用@api.model，表示这里的vals应该为一个。如果多个，通过这个装饰，也可以处理。
