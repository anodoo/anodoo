...
'''Anodoo
http://www.alanhou.org/odoo-13-orm-api/
 
l RecordCache
	n Model兼具DAO和数据集的功能
	n 它是单例的
	数据集合是一个还是多个是如何决定的？
	n Model方法中的self就是0,1或者多个，所有记录的集合
	n 字段集的读取
		u Record.name
		u Record.company_id.name
		u Record.name = ‘a’
		u Record[‘name’]
	n 字段集的数据都会一次性读取进来并缓存
				1. BaseModel
l 继承结构

第一个体系：元数据体系，type体系。
	n Api.Meta：元类，定义为一个type
		u MetaModel（只定义了两个方法）
			l Self._register
			l BaseModel / AbstractModel（主要是这个类，下面两个都是定义了几个属性）
				n Model
					u TransientModel(表在哪里？？何时清理，需要debug)
第二个体系：实体内的inherit体系
第三个体系：python的继承体系
    odoo首先根据元数据体系的定义，如odoo.addons.crm.models.crm_lead.Lead, 动态创建一个类ModelClass，odoo.api.crm.lead,
    该类的继承体系，即是inherit体系的定义，转为python的体系, 真实的类是：odoo.api.base《-odoo.api.ir.model.relation等，其中name：ir.model.relation
    
l 参考Modules/Loading.py L320
	n Load_modules 装载所有的modules
	n 调用registry中的setup_models, init_models
        n Setup
            u 安装各种相关的field
            u 先调用所有model的_prepare_setup()
            u 再调用所有model的_setup_base()
            u 再调用所有model的_setup_fields()
            u 最后调用所有model的_setup_complete()
        n Init
            u Registry L298 init_models
                l 每一个model为每一个database只有一个实例，会在load时进行init，对表结构，字段数据，约束进行处理
                l 分别调用model的_auto_init()， init()方法
                    n _auto_init()一般可以为当前实体创建索引，参考crm_lead.py
                l BaseModel中的init是空的，便于之类继承，人不需要改动_auto_init()
l BaseModel
	n 查询方法
		u Search / Read
			l Browse
				n 调用_browse
					u Object.__new__新建一个实例，也即recordset
					u 返回ids的记录集，因为仅仅是ids，所以占用很少
			l Search
				n 调用_search
					u 根据domain的list，以及其他参数查询
					u 调用_search
						l 查count
						l 或者查记录，仅仅查询ids
					u 调用browse（ids）创建记录集recordset
			l Search_count
				n 内部调用的search。
				n 仅仅select count(1)返回数量
			l Search_read
				n 调用search返回recordset
				n 调用read读指定的字段， 参考read				    
			l Read 将内容缓存，这个比较耗资源
				n 判断是否有读字段权限
				n 整理哪些需要读的字段，包括计算字段
				n 调用_read()
					u 读特定的字段
				n 处理结果，每一个元素为record，和字段名值字典。
			l Read_group
				n 按照group by
				n 调用_read_group_raw
		u Create
			l 通过api.model_create_multi装饰
			做一些处理
			l 调用_ceate 运行insert语句
			处理结果，将值设置到recordset中
			返回recordset
		u Write
			l 修改特定的字段的值
			recordset 中的值都会改
		...
		u Default_get
			l 特定的字段的默认值
				n 从context中找default_开头的默认值
				n Ir_default中存储默认值
				n 在fields定义中找default值
				n 在继承中找
		u Name_get, name_ceate, name_search
l Id
	n IdType L222
	n NewId
	n Origin_ids
...
l 手动实体和字段
	...
	n _pop_field
l View
	n Load_views 前台访问load_views获取视图
	n _get_default_from_view 等相关方法
	n _fields_view_get
	n Get_formview_id
l 权限相关
	n User_has_group
	n Check_access_rights
	n Check_access_rule
				2. Model
	n 只有一个实例？按理是一个进程一个实例的
	n Self是如何成为recordset的（参考类方法，实例方法）
		u  
l 继承模式
	n 都是继承Model，但是怎么实现类似Python的集成的呢？
 
o 瞬态模型基于models.TransientModel类，用于向导式的用户交互。 它们的数据仍然存储在数据库中，但期望是临时的。 一个清理作业定期从这些表中清除旧数据。 例如，从设置|翻译菜单打开的加载语言对话框窗口，它使用瞬态模型来存储用户选择和实现向导逻辑。
o  
o 抽象模型基于models.AbstractModel类,并且它不存储数据。 抽象模型作为可重用的功能集，利用Odoo的继承功能，混入到其他模型。 例如，mail.thread是一个抽象模型，由讨论插件提供，用于向其他模型添加消息和追踪关注功能。
 
 
models.Model
models.Abstract
